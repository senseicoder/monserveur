...
- [x] **Backup des données** — plan validé le 2026-08-02, **implémenté et testé de bout en bout le même jour** :
  ...
  - Périmètre : `/opt/mindwtr/data/{cloud,vaultwarden,ntfy}`, `/etc/letsencrypt`, `/opt/rat/data`, MariaDB hôte (`VOOSO`, `ttrss` — legacy MySQL, TT-RSS tourne maintenant sur Postgres, à vérifier si encore utilisée), Postgres `ttrss-docker_db`, dossiers home (`bot1`, `chatbot`, `chatbots`, `inbox0`, `oneclickpocket`, `mbox`).
  - **Reste à faire** : Cédric pose le cron `/etc/cron.d/backup-pull-pcloud` sur M6 (ligne prête, donnée en session) ; re-jouer `./run role backup-setup run` pour déployer le fix du marqueur FULL ; déclencher une fois `./run role backup-setup run --tags vhost-code-once` (backup **ponctuel** du code des vhosts, y compris `/space/www/apps/` — vrais docroots derrière des symlinks — et `sites-archive/relaibruno.daneel.net`, legacy conservé) ; supprimer le résidu vide `/opt/backup/staging` (ancienne version du rôle).
  - **Process de restauration complète à documenter et tester sur une VM locale au poste de dev** (VirtualBox sur ramoth2, déjà installé) — explicitement différé par Cédric à après la mise en place du backup lui-même.
