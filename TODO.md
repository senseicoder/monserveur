...
- [ ] **Backup des données** — plan validé le 2026-08-02, reste à implémenter :
  ...
  - Emplacement du rôle Ansible côté glaurung : `ansible/roles/backup-setup/` dans ce repo (nouveau `ansible/backup.list`, joué via `./run list backup.list run`, même convention que les autres rôles étroits). Correspond à la Phase 0 de [[glaurung-securite-plan]] (wiki) qui identifiait déjà ce même rôle comme prérequis n°1.
  ...
  - Périmètre : `/opt/mindwtr/data/cloud`, `/opt/mindwtr/data/vaultwarden`, `/opt/mindwtr/data/ntfy`, `/etc/letsencrypt`, `/opt/rat/data`, dump MariaDB hôte (bases `VOOSO` = lescoursdesophie.com, et `ttrss` = ancienne base MySQL TT-RSS — **à vérifier si encore utilisée, TT-RSS tourne maintenant sur Postgres**, cf. wiki `postes/glaurung.md` § Bases de données), dump Postgres du volume `ttrss-docker_db`, et les petits dossiers home (`bot1`, `chatbot`, `chatbots`, `inbox0`, `oneclickpocket`, `mbox`) — en attente de réintégration dans le projet domotique ou un futur projet bot, mais ont de la valeur en l'état.
  - Backup **ponctuel** (une seule fois, pas récurrent) du code de tous les vhosts actifs et archivés (y compris `sites-archive/relaibruno.daneel.net`, vhost legacy non documenté avant le scan du 2026-08-02, gardé pour le moment) — le code ne devrait plus changer.
